    ...
    try:
        ...
        try:
            ...
            if pull_result.returncode != 0:
                # ff-only failed — local and remote have diverged (e.g.
                # upstream force-pushed or rebase).  Local changes are
                # already stashed, so only committed work remains.
                # Prefer rebase to preserve local commits (e.g. cherry-picks
                # from agent sessions) instead of resetting them away.
                print(
                    "  ⚠ Fast-forward not possible (history diverged), rebasing local commits..."
                )
                ...
                if rebase_result.returncode != 0:
                    # Rebase failed — likely a conflict. Abort the rebase
                    # and fall back to reset so the update can still
                    # succeed. The old commits are preserved in reflog.
                    print(
                        "  ⚠ Rebase failed, falling back to reset..."
                    )
                    subprocess.run(
